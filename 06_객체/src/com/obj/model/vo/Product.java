...
public class Product {
	...
	// 7_객체 26/51 - 필드
	// 필드에 대한 접근 제한자는, 그 필드, 그 저장소를 어느 범위까지 이용할 수 있게 할것인지!
	// 생성된 인스턴스로 접근을 해서 이용을 해야함.
	// 클래스만 가지고는 안됨!
	// protected - 상속관계에 따라서 이용 범위를 정하는 것임 나중에 배움
	// default - 패키지 내에서만 사용하도록 설정
	// private - 외부에서 접근을 완전히 차단
	
	// 7_객체 28/51 - 예약어
	// 상수 사용할 때 final 사용! 이걸 필드에서 사용해보자. 보통 상수 사용할때는 그 코드 안에서만 지역적으로 사용하지만!
	// 필드는 힙영역에 생성됨. 힙영역에 생성된건 사라지지 않음. 여기서 final을 사용한다는 것은 광범위하게 사용할 값을 지정하는 것임.
	// 
}
